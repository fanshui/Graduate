...
//		SomeUtils.convertToTest();
		...
		new Thread() {
			public void run() {
				try {
					FileWriter fw = new FileWriter("mid/adjMatrix");
					BufferedWriter bw = new BufferedWriter(fw);
					bw.write(AdjMatrix.toString());
					bw.flush();
					bw.close();
					fw.close();
				} catch (IOException e) {
...
//		System.out.println("___________________________________________________");
		...
		// HkmeanArrData.outputKarrData();
		new Thread() {
			public void run() {
				File file = new File("mid/kmeansArray.data");
				StringBuffer sb = new StringBuffer();
				int num = (int) Adjacentmatrix.getInstance().getMatrix().getColumnCount();
				for (int i = 0; i < num; i++) {
					for (int j = 0; j < K; j++) {
						sb.append(MatrixOpera.array[i][j] + "    ");
						// System.out.print(MatrixOpera.array[i][j]+" ");
					}
					sb.append("\n");
					// System.out.println();
